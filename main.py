...
def test_expected_occupancy_start():
    ...
    py.plot(data, filename='expected_occup.html')
...
def plot_traj_log_likelihood():
    ...
    import plotly.offline as py
    import plotly.graph_objs as go
    trace = go.Scatter(x=x, y=scores, mode='markers')
    py.plot([trace], filename='beta.html')
...
def output_heat_map(g, occupancies, traj, start_state, dest_set, beta_hat=None,
    ...
    hm = go.Heatmap(z=o, zmin=zmin, zmax=zmax,
            name="log expected occupancy")
    if beta_hat is not None:
        hm.name += "(beta_hat={})".format(beta_hat)
...
def plot_all_heat_maps():
    ...
    for i in range(len(trajectory) - 1):
        ...
        occupancies = format_occ(infer_occupancies(g, traj, beta=beta_hat,
                dest_set={model_goal}))
        data1 = output_heat_map(g, occupancies, traj, start, dest_set={model_goal},
                beta_hat=beta_hat, zmin=-8, zmax=0)

        fixed_occupancies = format_occ(infer_occupancies(g, traj, beta=beta_fixed,
                dest_set={model_goal}))
        data2 = output_heat_map(g, fixed_occupancies, traj, start, dest_set={model_goal},
                beta_hat=beta_fixed, zmin=-8, zmax=0)

        true_occupancies = format_occ(infer_occupancies(g, traj, beta=true_beta,
                dest_set={goal}))
        data3 = output_heat_map(g, true_occupancies, traj, start, dest_set={goal},
                beta_hat=true_beta, zmin=-8, zmax=0)

        first_title = "wrong goal, beta_hat={} (MLE)".format(beta_hat)
        if abs(beta_hat - 11) < 1e-4:
            first_title += " (beta at max!)"
        elif abs(beta_hat - 0.8) < 1e-4:
            first_title += " (beta at min!)"

        fig = tools.make_subplots(rows=1, cols=3,
                subplot_titles=(
                    first_title,
                    "wrong goal, beta={} (Ziebart beta)".format(beta_fixed),
                    "correct goal, ground truth beta={}".format(true_beta)))
        fig['layout'].update(title="t={}".format(i+1))
        ...
        py.plot(fig, filename="output/{}.html".format(100+i))
...
def plot_all_heat_maps2():
    ...
    for i in range(len(trajectory) - 1):
        ...
        for t in data2:
            fig.append_trace(t, 1, 2)
        py.plot(fig, filename="output/{}.html".format(100+i),
            image='png', image_filename="output/{}.png".format(100+i),
